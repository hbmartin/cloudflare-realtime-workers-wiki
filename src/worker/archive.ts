...
async function processArchiveDisconnectTarget(env: Env, target: ArchiveDisconnectTarget) {
  ...
  try {
    const claimed = await env.DB.prepare(
      ...
      .bind(leaseUntil, claimedAt, target.page_id, target.content_epoch, claimedAt)
      .first<{
        workspace_id: string;
        room: string;
        attempts: number;
    ...
    const page = await env.DB.prepare(
      ...
      .bind(target.page_id, claimed.workspace_id)
      .first<{
        content_epoch: number;
    ...
    const stub = env.DOCUMENT.getByName(claimed.room, hint ? { locationHint: hint } : undefined);
...
export async function processArchiveDisconnectTargets(env: Env, targets: ArchiveDisconnectTarget[]) {
