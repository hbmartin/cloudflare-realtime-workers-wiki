  ...
  it("redacts nested data and survives hostile error objects", () => {
    ...
    expect(() =>
      logger.error(
        "test.hostile",
        "test",
        "Bearer abc crn_thismustberedacted person@example.com https://example.test/path?secret=yes",
    ...
    expect(record.message).toBe("Bearer [redacted] [redacted-secret] [redacted-email] https://example.test/path");
