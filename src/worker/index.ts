...
async function clientTelemetryReport(request: Request): Promise<ClientTelemetryReport> {
  const declaredLength = Number(request.headers.get("content-length") ?? 0);
  if (declaredLength > CLIENT_TELEMETRY_MAX_BYTES) {
    throw new HttpError(413, "telemetry_too_large", "The telemetry report is too large.");
  }
  const source = await request.text();
  if (new TextEncoder().encode(source).byteLength > CLIENT_TELEMETRY_MAX_BYTES) {
    throw new HttpError(413, "telemetry_too_large", "The telemetry report is too large.");
  }
  let body: Record<string, unknown>;
  try {
    body = object(JSON.parse(source));
  } catch {
...
app.post("/api/telemetry/client-errors", async (c) => {
  ...
  const report = await clientTelemetryReport(c.req.raw);
  ...
  async queue(batch: MessageBatch<DeliveryQueueMessage>, env: Env, context?: ExecutionContext) {
    await Promise.all(
      batch.messages.map(async (message) => {
        const correlationId =
          message.body && "correlationId" in message.body && message.body.correlationId
            ? message.body.correlationId
            : message.body && "outboxId" in message.body
