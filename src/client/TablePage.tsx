...
const EDITING_ACCESS_LOST_MESSAGE = "Editing access is no longer available. Reloaded the authoritative table.";
...
const REVISION_POLL_HEALTHY_JITTER_MS = 1_000;
...
class InvalidMutationResponseError extends Error {
  constructor() {
    super("A successful table mutation response did not include the expected revision.");
    this.name = "InvalidMutationResponseError";
  }
}

function mutationRevision(result: unknown, expectedRevision: number) {
  const revision = (result as { revision?: unknown } | null)?.revision;
  if (typeof revision !== "number" || !Number.isSafeInteger(revision) || revision !== expectedRevision + 1) {
...
export function TablePage({
  ...
  const markPageUnavailable = useCallback(
    (cause: ApiClientError) => {
      if (pageUnavailableRef.current || !mountedRef.current) return;
      pageUnavailableRef.current = true;
      revisionPollModeRef.current = "off";
      configureRevisionPollingRef.current("off");
      const currentLease = leaseTokenRef.current;
      clearLocalLease();
      if (currentLease) releaseTableLease(page.id, currentLease);
  ...
  const adoptAuthoritativeTable = useCallback(
    (
      ...
      setLoadError(null);
      if (!background) resetRevisionPollingBackoffRef.current();
  ...
  useEffect(() => {
    ...
    const resetBackoffAfterForegroundLoad = () => {
  ...
  useEffect(() => {
    ...
    const renewLease = async () => {
      ...
      try {
        ...
        if (!active) return;
        // A parsed response without a valid duration cannot establish a usable
        // deadline, so drop the lease. Transport and response-body failures
        // retry below against only the previously confirmed deadline; they
        // never extend it.
    ...
    const execute = async () => {
      ...
      while (leaseTokenRef.current === currentLease) {
        ...
        try {
          ...
          const committedRevision = mutationRevision(result, currentRevision);
          ...
          setTable((current) => {
            ...
            const revision = Math.max(current.revision, committedRevision);
            return revision === current.revision ? current : { ...current, revision };
          });
          return result;
        } catch (cause) {
          if (!isMounted()) return null;
