...
describe("TablePage", () => {
  ...
  it("uses the lease-renewal fallback for transport diagnostics", async () => {
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path.endsWith("/lease") && init?.method === "PATCH") {
        renewals += 1;
        if (renewals === 1) throw new TypeError("Failed to fetch");
        return { leaseDurationMs: LEASE_DURATION_MS };
      }
      return { table };
    });
  ...
  it("ends the lease when a renewal response has invalid timing", async () => {
    vi.useFakeTimers();
    vi.mocked(api).mockImplementation(async (path, init) => {
      if (path.endsWith("/lease") && init?.method === "POST") return leaseResult();
  ...
  it.each([
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      if (path.endsWith("/lease") && init?.method === "POST") return leaseResult();
      if (path.endsWith("/lease") && init?.method === "PATCH") throw responseError();
      return { table };
    });
    await renderActiveEditor();
    ...
    expect(screen.getByText("Editing lease active")).toBeInTheDocument();
    expect(screen.getByText("The editing lease could not be renewed. Retrying.")).toBeInTheDocument();
    expect(screen.queryByText(`${responseError().message} Retrying.`)).not.toBeInTheDocument();
    expect(api).not.toHaveBeenCalledWith("/api/tables/table-page/lease", expect.objectContaining({ method: "DELETE" }));
  ...
  it.each([
    ...
    [
      "is empty JSON",
      () => {
        throw new EmptyApiResponseError(200, {
          requestPath: "/api/tables/table-page/cells/row/status",
          responseUrl: null,
          contentType: "application/json",
        });
  ...
  it.each([
    ...
    [
      "an empty response without a content type",
      () =>
  ...
  ])("does not latch an unreachable revision after %s", async (_case, responseError) => {
    let loads = 0;
    let mutations = 0;
    vi.mocked(api).mockImplementation(async (path, init) => {
      if (path.endsWith("/lease") && init?.method === "POST") return leaseResult();
  ...
  it.each([404, 410])("stops polling when a mutation reports page-not-found with status %i", async (status) => {
    vi.useFakeTimers();
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path.endsWith("/lease") && init?.method === "DELETE") return { ok: true };
      if (path.includes("/cells/")) {
        throw new ApiClientError(status, "page_not_found", `Request failed (${status}).`, true);
      }
      loads += 1;
      return { table };
    });
    ...
    expect(screen.queryByText(`Request failed (${status}).`)).not.toBeInTheDocument();
    expect(api).toHaveBeenCalledWith("/api/tables/table-page/lease", {
      method: "DELETE",
      body: JSON.stringify({ leaseToken: "lease-token" }),
      keepalive: true,
  ...
  it("preserves a non-retryable lease-verification reason after a clock change", async () => {
    stubMonotonicClock();
    const acquiredAt = Date.now();
    vi.mocked(api).mockImplementation(async (path, init) => {
      if (path.endsWith("/lease") && init?.method === "POST") return leaseResult();
    ...
    expect(screen.getByText("Read-only")).toBeInTheDocument();
    expect(screen.getByText("Your workspace role is read-only.")).toBeInTheDocument();
    expect(
      screen.queryByText("Editing was paused because the lease could not be verified after the system clock changed."),
    ).not.toBeInTheDocument();
  });
  ...
  it("backs off and reports background rejections superseded by newly queued edits", async () => {
    ...
    for (const [index, failure] of retryFailures.entries()) {
      ...
      recoveryWarningCounts.push(screen.queryAllByText("The table depth could not be restored.").length);
    }
  ...
  it("wakes unknown-revision recovery after a mutation clears the last depth target", async () => {
    ...
    expect(screen.getByText("Editing lease active")).toBeInTheDocument();
    expect(screen.queryByText("Editing paused while table reloads")).not.toBeInTheDocument();
    expect(screen.queryByText("Table temporarily unavailable.")).not.toBeInTheDocument();
    expect(
      screen.queryByText("Table refresh is delayed while waiting for the latest revision."),
    ).not.toBeInTheDocument();
  ...
  it("reports a failed sorted-depth restore and keeps the revision invalidated", async () => {
    ...
    expect(await screen.findByText("The table depth could not be restored.")).toBeInTheDocument();
