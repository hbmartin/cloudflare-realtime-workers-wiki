...
vi.mock("./mentions", async (importOriginal) => ({
  ...
  invalidateAllPagePreviews: mocks.invalidateAllPagePreviews,
...
vi.mock("./retry", async (importOriginal) => ({
  ...
  waitForWorkspaceInvalidationRetry: mocks.waitForWorkspaceInvalidationRetry,
}));
...
describe("App error handling", () => {
  beforeEach(() => {
    ...
    mocks.waitForWorkspaceInvalidationRetry.mockReset();
    mocks.waitForWorkspaceInvalidationRetry.mockResolvedValue(undefined);
  });
  ...
  it("reports a create failure without an unhandled rejection", async () => {
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === "/api/pages" && init?.method === "POST") {
        throw new ApiClientError(409, "create_rejected", "Page creation was rejected.");
      }
      throw new Error(`Unexpected API request: ${path}`);
    });
    render(<App />);
  ...
  it("reports a move failure without an unhandled rejection", async () => {
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === `/api/pages/${secondPage.id}/move` && init?.method === "POST") {
        throw new ApiClientError(409, "move_rejected", "Page movement was rejected.");
      }
      throw new Error(`Unexpected API request: ${path}`);
    });
    render(<App />);
    ...
    expect(await screen.findByText("Page movement was rejected.")).toBeInTheDocument();
    expect(document.querySelector(".workspace-sidebar")).not.toHaveClass("open");
    await waitFor(() => expect(document.activeElement).toBe(openSidebar));
  });
  ...
  it("closes the mobile drawer and restores focus when archive fails", async () => {
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === "/api/pages/tree") return { pages: [page] };
      if (path === `/api/pages/${page.id}` && init?.method === "DELETE") {
        throw new ApiClientError(409, "archive_rejected", "Archive was rejected.");
      }
      throw new Error(`Unexpected API request: ${path}`);
    });
    render(<App />);
  ...
  it("reconciles an uncertain page creation and clears its diagnostics after observing the requested page", async () => {
    const createdPageId = "00000000-0000-4000-8000-000000000002";
    const createdPage = {
      ...page,
      id: createdPageId,
      position: "b0",
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === "/api/pages/tree") {
        treeLoads += 1;
        return { pages: treeLoads === 1 ? [page] : [page, createdPage] };
      }
      if (path === "/api/pages" && init?.method === "POST") throw networkError;
      throw new Error(`Unexpected API request: ${path}`);
    });
    render(<App />);
  ...
  it("does not accept an unrelated page as a create response or reconciliation result", async () => {
    const requestedPageId = "00000000-0000-4000-8000-000000000003";
    const unrelatedPage = { ...page, id: "unrelated-page", position: "b0", title: "Unrelated" };
    const randomUUID = vi.spyOn(crypto, "randomUUID").mockReturnValue(requestedPageId);
    const reported = vi.spyOn(console, "error").mockImplementation(() => undefined);
    onTestFinished(() => randomUUID.mockRestore());
    onTestFinished(() => reported.mockRestore());
    let treeLoads = 0;
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === "/api/pages/tree") {
        treeLoads += 1;
        return { pages: treeLoads === 1 ? [page] : [page, unrelatedPage] };
      }
      if (path === "/api/pages" && init?.method === "POST") return { page: unrelatedPage };
      throw new Error(`Unexpected API request: ${path}`);
    });
    render(<App />);
    ...
    expect(
      await screen.findByText("The page-creation result could not be verified after refreshing the page tree."),
    ).toBeInTheDocument();
    expect(treeLoads).toBe(3);
    expect(reported).toHaveBeenCalledWith(
      "Page creation result could not be verified",
      expect.objectContaining({ operationId: requestedPageId, outcome: "committed-invalid-response" }),
    );
  });

  it("does not claim a committed create failed when local processing and reconciliation fail", async () => {
    const createdPageId = "00000000-0000-4000-8000-000000000004";
    const createdPage = {
      ...page,
      id: createdPageId,
      position: "b0",
    ...
    const randomUUID = vi.spyOn(crypto, "randomUUID").mockReturnValue(createdPageId);
    const recordUpserts = vi.spyOn(PageLoadEventBuffer.prototype, "recordUpserts").mockImplementationOnce(() => {
      throw new Error("Page buffer unavailable.");
    });
    onTestFinished(() => randomUUID.mockRestore());
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === "/api/pages" && init?.method === "POST") return { page: createdPage };
      throw new Error(`Unexpected API request: ${path}`);
    });
    render(<App />);
    ...
    expect(
      await screen.findByText(
        "The page was created, but the workspace could not be updated. Tree refresh unavailable.",
      ),
    ).toBeInTheDocument();
  ...
  it("retries unavailable move receipts and accepts a confirming page-tree refresh", async () => {
    const movedPage = { ...page, revision: 2 };
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === "/api/pages/tree") {
        treeLoads += 1;
        return { pages: [treeLoads === 1 ? page : movedPage] };
      }
      if (path === `/api/pages/${page.id}/move` && init?.method === "POST") return { ok: true };
      if (path === `/api/pages/${page.id}/moves/${operationId}`) {
        receiptLoads += 1;
  ...
  it("accepts a reconciled move when a concurrent sibling is interposed beside it", async () => {
    const secondPage = { ...page, id: "second-page", position: "b0", title: "Second" };
    const thirdPage = { ...page, id: "third-page", position: "d0", title: "Third" };
    const interposedPage = { ...page, id: "interposed-page", position: "b5", title: "Interposed" };
    const movedPage = { ...page, position: "c0", revision: 2 };
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === "/api/pages/tree") {
        treeLoads += 1;
        return {
          pages: treeLoads === 1 ? [page, secondPage, thirdPage] : [secondPage, interposedPage, movedPage, thirdPage],
        };
      }
      if (path === `/api/pages/${page.id}/move` && init?.method === "POST") return { ok: true };
      if (path === `/api/pages/${page.id}/moves/${operationId}`) {
  ...
  it("reports an uncertain move as not committed after two fresh missing receipts", async () => {
    ...
    expect(await screen.findByText("The page was not moved.")).toBeInTheDocument();
    expect(treeLoads).toBe(2);
    expect(receiptLoads).toBe(2);
    expect(mocks.waitForReconciliationRetry).toHaveBeenCalledOnce();
    expect(reported).toHaveBeenCalledWith(
  ...
  it("keeps a missing-receipt result uncertain when the page was concurrently deleted", async () => {
    ...
    expect(screen.queryByText("The page was not moved.")).not.toBeInTheDocument();
    expect(treeLoads).toBe(2);
    expect(receiptLoads).toBe(2);
  });

  it("keeps failures from overlapping create and move attempts", async () => {
    const secondPage = { ...page, id: "second-page", position: "b0", title: "Second" };
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === `/api/pages/${secondPage.id}/move` && init?.method === "POST") return movement.promise;
      throw new Error(`Unexpected API request: ${path}`);
    });
    render(<App />);
    ...
    await act(async () => {
      movement.reject(new ApiClientError(409, "move_rejected", "Concurrent move failed."));
      await movement.promise.catch(() => undefined);
    });
    await act(async () => {
      creation.reject(new ApiClientError(409, "create_rejected", "Concurrent creation failed."));
      await creation.promise.catch(() => undefined);
    });

    const alert = await screen.findByRole("alert");
    expect(alert).toHaveTextContent("Concurrent move failed.");
    expect(alert).toHaveTextContent("Concurrent creation failed.");
  });

  it("does not clear a reported create warning when a different mutation starts", async () => {
    const secondPage = { ...page, id: "second-page", position: "b0", title: "Second" };
    const requestedPageId = "00000000-0000-4000-8000-000000000005";
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === "/api/pages/tree") {
        ...
        if (treeLoads === 2) return reconciliation.promise;
        return { pages: [page, secondPage] };
      }
      if (path === "/api/pages" && init?.method === "POST") throw new TypeError("Failed to fetch");
      if (path === `/api/pages/${secondPage.id}/move` && init?.method === "POST") return movement.promise;
    ...
    fireEvent.keyDown(secondLink, { altKey: true, key: "ArrowUp" });
    expect(
      screen.getByText("The page-creation result could not be verified. Checking the page tree."),
    ).toBeInTheDocument();
    ...
    const alert = await screen.findByRole("alert");
    expect(alert).toHaveTextContent("Concurrent move failed.");
    expect(alert).toHaveTextContent("page-creation result could not be verified");
  ...
  it("suppresses an older failure after a newer move of the same page succeeds", async () => {
    const secondPage = { ...page, id: "second-page", position: "b0", title: "Second" };
    const movedPage = { ...secondPage, position: "A0", revision: 2 };
    const olderMove = deferred<{ page: Page }>();
    const newerMove = deferred<{ page: Page }>();
    let moves = 0;
    vi.mocked(api).mockImplementation(async (path, init) => {
      if (path === "/api/install") return { initialized: true };
      if (path === "/api/security/status")
      ...
      if (path === `/api/pages/${secondPage.id}/move` && init?.method === "POST") {
        moves += 1;
        return moves === 1 ? olderMove.promise : newerMove.promise;
      }
      throw new Error(`Unexpected API request: ${path}`);
    });
    render(<App />);
    ...
    await act(async () => {
      newerMove.resolve({ page: movedPage });
      await newerMove.promise;
    });
    await act(async () => {
      olderMove.reject(new ApiClientError(409, "move_rejected", "Superseded move failed."));
      await olderMove.promise.catch(() => undefined);
    });

    expect(screen.queryByText("Superseded move failed.")).not.toBeInTheDocument();
  ...
  it("stops move processing when the workspace unmounts before the mutation settles", async () => {
    const secondPage = { ...page, id: "second-page", position: "b0", title: "Second" };
    const movedPage = { ...secondPage, position: "A0" };
    const movement = deferred<{ page: Page }>();
    const recordUpserts = vi.spyOn(PageLoadEventBuffer.prototype, "recordUpserts");
    onTestFinished(() => recordUpserts.mockRestore());
    vi.mocked(api).mockImplementation(async (path, init) => {
    ...
    app.unmount();
    await act(async () => {
      movement.resolve({ page: movedPage });
  ...
  it("invalidates committed archive previews but stops workspace processing after unmount", async () => {
  ...
  it("invalidates restored previews but starts no workspace refresh after unmount", async () => {
    const archivedPage = { ...page, archivedAt: 2 };
    const restoreResponse = deferred<{ pages: Page[] }>();
    let treeLoads = 0;
    let trashLoads = 0;
    vi.mocked(api).mockImplementation(async (path, init) => {
      if (path === "/api/install") return { initialized: true };
      ...
      if (path === `/api/pages/${page.id}/restore` && init?.method === "POST") return restoreResponse.promise;
      throw new Error(`Unexpected API request: ${path}`);
    });
    const app = render(<App />);
    ...
    await waitFor(() =>
      expect(api).toHaveBeenCalledWith(`/api/pages/${page.id}/restore`, expect.objectContaining({ method: "POST" })),
    );

    app.unmount();
    await act(async () => {
      restoreResponse.resolve({ pages: [page] });
  ...
  it("invalidates permanently deleted previews but starts no workspace refresh after unmount", async () => {
    const owner = { ...member, role: "owner" as const };
    const archivedPage = { ...page, archivedAt: 2 };
    const deleteResponse = deferred<{ ok: true; pageIds: string[] }>();
    let treeLoads = 0;
    let trashLoads = 0;
    vi.stubGlobal(
      "confirm",
    ...
    await waitFor(() =>
      expect(api).toHaveBeenCalledWith(
        `/api/pages/${page.id}/permanent-delete`,
    ...
    app.unmount();
    await act(async () => {
      deleteResponse.resolve({ ok: true, pageIds: [page.id] });
  ...
  it("does not report recovery reconciliation cancellation after workspace teardown", async () => {
    const reported = vi.spyOn(console, "error").mockImplementation(() => undefined);
    onTestFinished(() => reported.mockRestore());
    let treeLoads = 0;
    vi.stubGlobal(
    ...
    vi.mocked(api).mockImplementation(async (path, init) => {
      ...
      if (path === "/api/pages/tree") {
        ...
        if (treeLoads === 1) return { pages: [page] };
        const signal = init?.signal;
        if (!signal) throw new Error("The recovery request did not receive a signal.");
        return new Promise<never>((_resolve, reject) => {
          signal.addEventListener("abort", () => reject(signal.reason), { once: true });
        });
      }
  ...
  it("invalidates derived page data when the workspace must be resynced", async () => {
    let treeLoads = 0;
    let unreadLoads = 0;
    vi.mocked(api).mockImplementation(async (path) => {
      if (path === "/api/install") return { initialized: true };
    ...
    await waitFor(() => expect(treeLoads).toBe(2));
    await waitFor(() => expect(unreadLoads).toBe(2));
    expect(mocks.invalidateAllPagePreviews).toHaveBeenCalledOnce();
    expect(mocks.editorRender.mock.calls.at(-1)?.[0]).toMatchObject({ backlinksRevision: 1 });
    expect(mocks.waitForReconciliationRetry).not.toHaveBeenCalled();
  });
  ...
  it("keeps retrying an invalidated workspace until a tree observation succeeds", async () => {
  ...
  it.each([
    ...
    let treeLoads = 0;
    vi.mocked(api).mockImplementation(async (path, init) => {
      if (path === "/api/pages/tree" && ++treeLoads === 1 && retry) throw new Error("Tree unavailable");
      if (path === `/api/pages/${hiddenPage.id}`) return { page: hiddenPage, sidebarHidden: true };
      return shellApi(path, init);
    });
    render(<App />);
  ...
  it.each([
    ...
    const shellApi = vi.mocked(api).getMockImplementation()!;
    vi.mocked(api).mockImplementation(async (path, init) => {
      if (path === `/api/pages/${unavailable.id}`) return { page: unavailable, sidebarHidden: false };
      return shellApi(path, init);
    });
    render(<App />);
  ...
  it("reports an uncertain permanent-delete result without claiming the deletion failed", async () => {
    ...
    expect(reported).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ outcome: "uncertain", pageId: page.id, errorMessage: "Failed to fetch" }),
    );
  });
  ...
  it("switches spaces and manages favorites, pins, and page tags", async () => {
    ...
    expect(scrollRegion).toContainElement(switcher);
    expect(scrollRegion).toContainElement(screen.getByRole("navigation"));
    expect(scrollRegion).toContainElement(favoritesSection);
