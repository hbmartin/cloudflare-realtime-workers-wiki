...
A password or fresh session-bound Slack primary proof plus an active recovery code creates a ten-minute recovery session. Other sessions, browser trust, and the remaining recovery codes are revoked. The response displays a random recovery resume key once; only its hash is stored. The session can use only key acknowledgment, security status, recovery resume, and sign-out until it sends the exact displayed key to the server. A pending key handoff expires after ten minutes. The user must restore a factor and save new recovery codes before accessing the workspace.

If the ten-minute grant expires, the original live recovery session can resume enrollment after primary-factor re-entry. A replacement sign-in requires both a fresh primary factor and the previously acknowledged key. Every successful resume displays a new key for acknowledgment, but the old acknowledged key remains usable with fresh primary proof until the new key is acknowledged. A later successful resume supersedes the earlier pending handoff and its session. A failed replacement session is cleaned up, with durable retry if cleanup cannot finish immediately. If the initial key was lost before acknowledgment, only the original recovery session can request another with fresh primary proof; another session needs an operator reset. The original 24-hour recovery deadline never moves. Factor restoration, another recovery, or an operator reset revokes both pending and active keys.

Deploy migration `0043_slack_delivery_recovery_health.sql` before releasing this recovery flow. It adds the pending key handoff and durable replacement-session cleanup state. The scheduled maintenance task clears expired handoffs and unfinished replacement sessions.
