...
| Slack sign-in is hidden or Settings reports `users:read` missing                                   | The bot installation predates the secure identity scope set                                                 | Have an owner use **Reauthorize Slack**, then verify scope health in Settings               |
| Slack sign-in says the account is not linked                                                       | A local account with the same email exists, but implicit account linking is disabled                        | Sign in normally, verify a security factor, then use **Connect Slack identity** in Settings |
| Slack sign-in rejects a guest or Slack Connect member                                              | Only active full members of the bound Slack team may authenticate                                           | Use an eligible full-member Slack account or sign in with a local credential                |
...
| Message                                                      | Meaning                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Scheduled deletion cleanup failed`                          | The scheduled deletion pass threw. Check `deletion_jobs`                                                                                                                                                                                                                 |
| `Scheduled archive disconnect failed`                        | The scheduled archive pass threw. Check `archive_disconnect_targets`                                                                                                                                                                                                     |
| `Failed to prune page move receipts`                         | The scheduled seven-day move-receipt retention pass threw. Check D1 availability and the `page_move_receipts` index                                                                                                                                                      |
| `Page move receipt pruning reached its catch-up limit`       | Ten batches deleted 10000 expired receipts, but more may remain. Check the expired-row count and whether move volume is staying above the cleanup capacity                                                                                                               |
...
| `Outbox sweep cap reached; scheduling continuation`          | One pass enqueued 250 rows and queued another sweep so the backlog continues draining without waiting for cron                                                                                                                                                           |
| `Outbox sweep continuation enqueue failed`                   | The follow-up sweep message could not be queued. The remaining rows are durable and the next cron will retry them                                                                                                                                                        |
...
4. Whether the cron has run since the incident began.
