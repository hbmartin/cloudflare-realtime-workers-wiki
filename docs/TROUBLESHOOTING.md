...
`table_revision_failed` are exceptions because there is no metric carrying the response code. Search Workers
...
`moveError*`; receipt failures use `receiptError*`. `Table revision could not be advanced` carries the page id,
