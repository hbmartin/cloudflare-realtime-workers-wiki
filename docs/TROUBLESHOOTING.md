...
| `Page move batch result was invalid or inconsistent.`         | D1 returned malformed metadata or contradictory state. Inspect `moveError*`. `recoveredFromReceipt`: true = recovered `200`; false = failure. Match the operation id to receipt logs: unreadable = `503` (retry same id), conflicting = `409`, none = `500`              |
