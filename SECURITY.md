...
- Private user access also requires an enrolled factor and server-owned session assurance. Passkeys require verified user presence and PIN/biometric verification; browser trust has a fixed, revocable 30-day deadline.
- No account can disable protection or remove its last factor. Factor changes require recent verification; password-only sessions cannot manage enrolled accounts.
...
See [mandatory account protection](docs/ACCOUNT_SECURITY.md) for operator resets, secret preservation, deployment checks, and the five-minute realtime revocation bound. Anonymous public shares are intentionally outside account authentication.
