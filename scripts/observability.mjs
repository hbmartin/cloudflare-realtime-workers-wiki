...
async function graphqlMetrics(accountId, token, queueId, startedAt, fetcher = fetch) {
  const query = `query Observability($account: string!, $start: Time!, $workflowStart: Time!, $end: Time!, $script: string!, $workflow: string!, $queue: string!) {
    viewer { accounts(filter: { accountTag: $account }) {
      ...
      workflow: workflowsAdaptive(limit: 10000, filter: { workflowName: $workflow, datetime_geq: $workflowStart, datetime_leq: $end }, orderBy: [datetime_ASC]) {
    ...
    "https://api.cloudflare.com/client/v4/graphql",
    {
      ...
      body: JSON.stringify({
        ...
        variables: {
          ...
          workflowStart: new Date(Math.min(startedAt.getTime(), Date.now() - 31 * 60_000)).toISOString(),
...
export function evaluateThresholds(snapshot, timestamp = Date.now()) {
  ...
  const oldest = number(snapshot.delivery?.oldest_message_timestamp_ms);
  if (oldest > 0 && timestamp - oldest > 5 * 60_000) failures.push("queue_oldest_message_high");
  ...
  for (const row of snapshot.workflow) {
    ...
    if (!instanceId || !String(eventType).startsWith("WORKFLOW_") || !Number.isFinite(datetime)) continue;
  ...
  if (
    snapshot.workflow.some((row) =>
      ["WORKFLOW_FAILURE", "WORKFLOW_INTERNAL_ERROR", "ROLLBACK_FAILED", "ROLLBACK_ATTEMPT_FAILURE"].includes(
        row.eventType ?? row.dimensions?.eventType,
      ),
    )
  ) {
...
function markdown(snapshot, failures, windows, title = "Cloudflare observability monitor") {
  ...
  return [
    ...
    `- Delivery backlog / DLQ: ${number(snapshot.delivery?.backlog_count)} / ${number(snapshot.dlq?.backlog_count)}`,
